...
pcas$roation^2
## get the x-values of PCAs and make it data frame
...
## Get rid of NAs
## Only use na.omit when you havge specifically selected for the variables you want to include in the model
noNAs <- na.omit(starwars)

noNAs <- filter (starwars, !is,na(mass), !is.na(height))

#replace with means
replaceWithMeans<- mutate(starwars, mass = ifelse(is.na(mass),
                                                  mean(mass),
                                                  mass))
...
irisCors <- irisAllNumeric |> cor(irisAllNumeric) |>
  melt() |> 
  as.data.frame()
...
# Choose 70-85% of data to train
library(rsample)

##Set a seed for reproductibnility
set.seed(71723)

##Regression dataset splits
#Create a split
reg_split <- initial_split(irisAllNumeric, prop = .75) ## Put 75% of data into the training set
...
##Classification datat splits (use iris instead of irisAllNumeric)
...
# Calcualte errors for regression
library(yardstick)

#lm_fit, boost_reg_fit, forest_ref_fit
